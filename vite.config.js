  ...
  const concatenateJavaScript = () => {
    ...
    return {
      ...
      async generateBundle(options, bundle) {
        ...
        console.log('📋 Iniciando concatenación de archivos...');
        filesToConcat.forEach((filePath) => {
          ...
          if (fs.existsSync(fullPath)) {
            ...
            concatenatedContent += '\n';
            console.log(`✅ Concatenado: ${filePath} (${(fileSize/1024).toFixed(1)}KB)`);
          } else {
            console.warn(`⚠️ Archivo no encontrado: ${fullPath}`);
          }
        });
        
        console.log(`📊 Concatenación completada: ${filesProcessed} archivos, ${(totalSize/1024).toFixed(1)}KB total`);
...
`;
        ...
        try {
          ...
          console.log('🔄 Minificando all.js (con console.log, sin mangle)...');
          const minifyStartDev = Date.now();
          ...
          const minifyTimeDev = Date.now() - minifyStartDev;
          const devSize = minifiedDev.code?.length || originalSize;
          console.log(`✅ all.js minificado: ${(devSize/1024).toFixed(1)}KB (${minifyTimeDev}ms)`);

          // all.min.js - minificado completamente para producción
          console.log('🔄 Minificando all.min.js (producción completa)...');
          const minifyStartProd = Date.now();
          ...
          const minifyTimeProd = Date.now() - minifyStartProd;
          const prodSize = minifiedProd.code?.length || originalSize;
          console.log(`✅ all.min.js minificado: ${(prodSize/1024).toFixed(1)}KB (${minifyTimeProd}ms)`);
          ...
          const prodReduction = ((originalSize - prodSize) / originalSize * 100).toFixed(1);
          
          console.log('📊 RESUMEN DE MINIFICACIÓN:');
          console.log(`   📁 Archivos procesados: ${filesProcessed}`);
          console.log(`   📏 Tamaño original: ${(originalSize/1024).toFixed(1)}KB`);
          console.log(`   🔧 all.js: ${(devSize/1024).toFixed(1)}KB (-${devReduction}%)`);
          console.log(`   🚀 all.min.js: ${(prodSize/1024).toFixed(1)}KB (-${prodReduction}%)`);
          console.log(`   ⏱️ Tiempo total: ${totalTime}ms`);
          console.log('✅ all.js (minificado con map) y all.min.js (producción) generados');
        } catch (error) {
