...
/**
 * Enqueue assets específicos para Servicios block (Frontend)
 */
function amentum_enqueue_servicios_block_assets() {
    // Solo cargar si el bloque está presente en la página
    if (has_block('amentum/servicios')) {
        wp_enqueue_style(
            'amentum-servicios-style',
            get_template_directory_uri() . '/blocks/servicios/style.css',
            array(),
            wp_get_theme()->get('Version')
        );
    }
}
add_action('wp_enqueue_scripts', 'amentum_enqueue_servicios_block_assets');

/**
 * Enqueue editor assets para Servicios block (Backend)
 */
function amentum_enqueue_servicios_block_editor_assets() {
    // JS del editor
    wp_enqueue_script(
        'amentum-servicios-editor',
        get_template_directory_uri() . '/blocks/servicios/editor.js',
    ...
    wp_enqueue_style(
        'amentum-servicios-editor-style',
        get_template_directory_uri() . '/blocks/servicios/style.css',
        array(),
        wp_get_theme()->get('Version')
    );
}
add_action('enqueue_block_editor_assets', 'amentum_enqueue_servicios_block_editor_assets');